...
void OutputPin::removeconnection(ApplicationManager *pmanager)
{
	int copym_Conn=m_Conn;
	for(int i=0;i<copym_Conn;i++)
	{
		pmanager->deletecomp(m_Connections[i]);
	}
}
void OutputPin::modify(Connection* c)
{
	for(int i=0;i<m_Conn;i++)
		if(c==m_Connections[i])
		{
